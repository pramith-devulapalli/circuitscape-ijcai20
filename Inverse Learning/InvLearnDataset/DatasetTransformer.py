...
class DatasetTransformer:
    ...
    def split_train_test(self, df, ratio, seed=42):
        np.random.seed(seed)
        nRows = int(len(df)*ratio)
        df_shuffled = df.sample(frac=1)
        test_df = df_shuffled.iloc[:nRows].reset_index(drop=True)
        train_df = df_shuffled.iloc[nRows:].reset_index(drop=True)
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.ratio != 0.0:
            train_df, test_df = self.split_train_test(hitting_times_dataframe, self.ratio)
